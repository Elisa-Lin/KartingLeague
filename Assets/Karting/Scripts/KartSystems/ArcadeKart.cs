...
namespace KartGame.KartSystems
{
    ...
    public class ArcadeKart : MonoBehaviour
    {
        ...
        private PickupObject[] pickupObjects;
        private List<ObjectiveRecord> records = new();
        private int previousPosition = -1;

        public void UpdateRecords(ObjectiveRecord newRecord)
        {
            Debug.Log($"New record for {PlayerName} comes: {JsonConvert.SerializeObject(newRecord)}");
            records.Add(newRecord);

            Debug.Log($"{PlayerName} pickup list: {JsonConvert.SerializeObject(pickupObjects.Select(po => po.gameObject.name))}");
            
            var currentPosition = Array.IndexOf(pickupObjects, pickupObjects.First(po => po.gameObject.name == newRecord.Name));
            Debug.Log($"{PlayerName} current position: {currentPosition}; previous: {previousPosition}");
            if (currentPosition == (previousPosition + 1) % pickupObjects.Length)
            {
                Score += 10;
                previousPosition = currentPosition;
            }
            TimeCost = newRecord.Time;

            //Score = records.Count;
            Debug.Log($"{PlayerName} score updated: {Score}");
        }
        ...
        void Awake()
        {
            ...
            pickupObjects = FindObjectsOfType<PickupObject>().OrderBy(obj => obj.transform.GetSiblingIndex()).ToArray();
