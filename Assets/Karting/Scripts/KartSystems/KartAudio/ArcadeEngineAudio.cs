    ...
    public class ArcadeEngineAudio : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (arcadeKart != null)
            {
                ...
                Drift.volume = 0.0f;// arcadeKart.IsDrifting && arcadeKart.GroundPercent > 0.0f ? arcadeKart.Rigidbody.velocity.magnitude / arcadeKart.GetMaxSpeed() : 0.0f;
            }

            IdleSound.volume = 0.0f;// Mathf.Lerp(0.6f, 0.0f, kartSpeed * 4);

            if (kartSpeed < 0.0f)
            {
                // In reverse
                RunningSound.volume = 0.0f;
                ReverseSound.volume = 0.0f;// Mathf.Lerp(0.1f, ReverseSoundMaxVolume, -kartSpeed * 1.2f);
            ...
            else
            {
                ...
                RunningSound.volume = 0.0f;//Mathf.Lerp(0.1f, RunningSoundMaxVolume, kartSpeed * 1.2f);
